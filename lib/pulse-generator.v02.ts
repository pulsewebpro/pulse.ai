...
import { plantProjectV02, PulseSkinV02, PulseSector } from "./pulse-kit.v02";

export async function generateWebZipV02(opts: { userPrompt: string; skin: PulseSkinV02; sector: PulseSector; }): Promise<string> {
  const id = uuidv4();
  const outDir = path.join("/tmp", id);
  const zipPath = path.join("/tmp", `${id}.zip`);
  fs.mkdirSync(outDir, { recursive: true });

  plantProjectV02(outDir, opts.skin, opts.sector, opts.userPrompt);
