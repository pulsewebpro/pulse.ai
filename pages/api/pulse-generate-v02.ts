...
import { generateWebZipV02 } from "../../lib/pulse-generator.v02";

export const config = { api: { bodyParser: { sizeLimit: "1mb" } } };

const SKINS = ["crystal-oro","warm-cafe","minimal-noir"];
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const url = await generateWebZipV02({ userPrompt: String(prompt), skin, sector });
    res.status(200).json({ downloadUrl: url });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Error generando web (v0.2)" });
  }
}
