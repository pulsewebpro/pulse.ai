...
export default function ChatPulse({ size = 220, mood = "neutral", className = "", delay = 0.12 }: Props) {
  ...
  const variants = {
    hidden: { opacity: 0, y: prefersReducedMotion ? 0 : 28, scale: prefersReducedMotion ? 1 : 0.96, filter: "blur(2px)" },
    show:   { opacity: 1, y: 0, scale: 1, filter: "blur(0px)", transition: { duration: prefersReducedMotion ? 0.2 : 0.8, ease: [0.22,1,0.36,1], delay } },
    hover:  { y: prefersReducedMotion ? 0 : -4, transition: { duration: 0.5, ease: "easeOut" } }
  };
      ...
      <style jsx>{`
        ...
        .pulse-ring{
          ...
          box-shadow:
            ...
            0 18px 60px rgba(56,189,248,0.28),
            0 -8px 30px rgba(56,189,248,0.14),
            inset 0 0 40px rgba(255,215,0,0.10);
          pointer-events:none;
        }
        @media (max-width: 980px){
          .pulse-avatar{ transform-origin:left top; }
        }
        @media (prefers-reduced-motion: reduce){
          .pulse-ring{ box-shadow: 0 0 0 1px rgba(255,215,0,0.30); }
        }
      `}</style>
    </>
  );
